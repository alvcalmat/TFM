...
%% Callbacks: newTaskList, positionCallback, batteryCallback, stateCallback, missionOverCallback, beaconCallback. Solo actualizan el valor de variables internas.
%% checkBeaconTimeout comprueba si ha habido timeout o no, vacía la cola si sí y actualiza el valor del flag interno.
% isBatteryEnough comprueba si hay batería suficiente para terminar la tarea actual, vacía la cola si no la hay y actualiza el valor del flag interno.
% El diseño tanto del árbol de comportamiento como del noddo \emph{Agent Behavior Manager} en general se ha hecho con la intencion de concentrar la mínima inteliegencia necesaria en él que asegure el éxito de la misión y la seguridad del equipo y los trabajadores. 
% Decisiones (solo la de la batería, el resto en planner)
% Replanificaciones.
% Que hacen los callbacks
