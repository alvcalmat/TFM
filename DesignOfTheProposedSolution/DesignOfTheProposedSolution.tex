...
% En caso de que la conexión entre ambos nodos siga activa pero no se tenga batería suficiente, el objetivo del plan de emergencia es eliminar riesgos sobre el UAV mientras el \emph{High-Level Planner} genera instrucciones nuevas. Además, es probable que el nuevo plan involucre como primer paso recargar la batería. Por otro lado, existe la posibilidad de que en este momento se pierda la conexión.

% El peligro de la pérdida de conexión es que el \emph{High-Level Planner} replanificará la misión sin contar con el ACW desconectado, por lo que las tareas antes asignadas a él serán ejecutadas ahora por otros. Si en este escenario el ACW desconectado sigue con el último plan asignado, se podrían producir colisiones por ejemplo. El protocolo de emergencia asegura que el UAV desconectado no interfiera en los nuevos planes. Además, de esta forma se aprovecha el tiempo que transcurra hasta que la conexión se reestablezca recargando la batería, lo cual es positivo para la misión.
