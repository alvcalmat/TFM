...
% Como se ha dicho antes, el \emph{High-Level Planner} es un módulo centralizado que se ejecuta en una estación en tierra y constituye el bloque cognitivo principal de la arquitectura de software de la que esta proyecto forma parte. Su misión es planificar la misión de forma óptima, es decir, repartir las tareas pendientes entre los \glspl{ACW} disponibles especificando el orden en que se ejecutan y teniendo en cuenta el tiempo que se tarda en completar cada una, el tipo de cada \gls{UAV}, la distancia que tendrá que recorrer cada uno, la batería de la que disponen, la tarea que estaba ejecutando cada uno, la prioridad de cada tarea, la batería que consume cada tarea, las recargas que se necesitarán y en qué momento es mejor realizar las recargas.

%% Explicación y Pseudocódigo general de como se inicializa el nodo planner, el bucle while(ros::ok) y como se cirra cuando mission_over. Incluir escucha a imprevistos.
...
%% Enlazar lo de los callback con el párrafo de abajo y luego explicar los pseudocódigos de los Callback que se crean convenientes.
%% La información se gestiona principalmente mediante mapas y listas.

% Dado que el entorno donde operan los \glspl{UAv} es dinámico, este módulo ha sido programado de forma que pueda reaccionar ante algún imprevisto y recalcular el plan óptimo. Las situaciones en las que se ha considerado necesaria una replanificación están listadas en la sección \ref{sec:TaskReplanningSituations}. Las comunicaciones resumidas en las tablas \ref{tab:interfaces} y \ref{tab:shareddata} y en la figura \ref{fig:NodeDiagram} son suficientes para detectar estos imprevistos y poder responder a ellos de la mejor forma posible.

%% Explicar el psudocógigo genérico de cómo se reacciona ante un imprevisto (en el psudo, cuando se diga "Sucede un imprevisto", listar los eventos que desencadenan los imprevistos listados en el 3.4)

%% Explicar los pseudocódigos de los Callback que se crean convenientes:
%% Llegada de balizas y la reacción (pseudocódigo)

%% Explicar como se realiza la planificación y poner psudocódigo de cómo se lleva a cabo
